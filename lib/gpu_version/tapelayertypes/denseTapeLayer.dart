...
class DenseReluTL extends TapeLayer {
  ...
  GPUTensor<dynamic> forward(GPUTensor<dynamic> input, CommandBuffer tape, List<GPUTensor> intermediates) {
    ...
    // Call the fused kernel. It handles MatMul, Bias Add, and ReLU in one step.
    // We pass 'intermediates' so the hidden pre-relu tensor is garbage collected.
    GPUTensor<Matrix> out = matMulBiasReluGPU(typedInput, weights, bias, tape, intermediates);
