...
class SafetensorsLoaderBF16 {
  Map<String, dynamic> header = <String, dynamic>{};
  late Uint8List rawBytes;
  int bufferOffset = 0;

  SafetensorsLoaderBF16(String filePath) {
  ...
  void loadIntoTensors(Map<String, GPUTensor> modelTensors) {
    ...
    for (int i = 0; i < keys.length; i = i + 1) {
      ...
      if (dtype == 'BF16') {
        floatCount = byteLength ~/ 2;
        ByteData converter = ByteData(4);

        for (int j = 0; j < floatCount; j = j + 1) {
          int bf16Bits = tensorData.getUint16(j * 2, Endian.little);
          converter.setUint32(0, bf16Bits << 16, Endian.host);
          values.add(converter.getFloat32(0, Endian.host));
        }
      } else if (dtype == 'F32') {
        floatCount = byteLength ~/ 4;
...
void main() {
  printSafetensorsStructure('gemma/model.safetensors');
}
