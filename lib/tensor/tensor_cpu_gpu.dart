...
Tensor<T> encapsulateGPUGraph<T>(
  ...
  GPUEngine.retrieve(gpuOutput.id, out.dataPtr);
...
Tensor<T> executeGPUGraph<T>(
  ...
  for (int i = 0; i < cpuInputs.length; i = i + 1) {
    GPUEngine.load(gpuInputs[i].id, cpuInputs[i].dataPtr, gpuInputs[i].shape);
  }

  // 2. Execute the pre-compiled Forward Tape
  GPUEngine.run(forwardTape.bytes());
  ...
  out.creator = Node(
    cpuInputs,
      ...
      GPUEngine.run(backwardTape.bytes());
      ...
      for (int i = 0; i < cpuInputs.length; i = i + 1) {
        ...
        Pointer<Float> tempGradPtr = calloc<Float>(numElements);
        GPUEngine.retrieve('${gpuInputs[i].id}_grad', tempGradPtr);

        // INSTANT NATIVE ADDITION (Bypasses Dart GC and Loop entirely)
        GPUEngine.addPointers(cpuInputs[i].gradPtr, tempGradPtr, numElements);
